...
export async function getViews() {
  const res = await withLoading(fetch(`${apiBase}/api/views`));
...
export async function getSeedColumns(viewIds = [], lang = 'en') {
  ...
  qs.set('lang', lang);
  const res = await withLoading(fetch(`${apiBase}/api/seed/columns?${qs.toString()}`));
...
export async function getSeedValues({ col, term = '', viewIds = [], lang = 'en' }) {
  ...
  if (viewIds.length) qs.set('viewIds', viewIds.join(','));
  const res = await withLoading(fetch(`${apiBase}/api/seed/values?${qs.toString()}`));
  if (!res.ok) throw new Error(`seed values ${res.status}`);
  return res.json();
}

export async function traverseStepMulti(payload) {
  const res = await withLoading(fetch(`${apiBase}/api/traverseStepMulti`, {
