...
function renderSeedList() {
  ...
  state.seeds.forEach((s, idx) => {
    ...
    li.className = 'hint'; li.style.margin = '2px 0';
...
export async function refreshSeedColumns() {
  try {
    ...
    if (Array.isArray(cols) && cols.length) {
      ...
      def.value = ''; def.textContent = '--'; def.selected = true; sel.appendChild(def);
    }
    (cols || []).forEach(item => {
      const opt = document.createElement('option');
      if (item && typeof item === 'object' && 'id' in item) {
        opt.value = String(item.id); opt.textContent = String(item.text ?? item.id);
      } else { const c = String(item ?? ''); opt.value = c; opt.textContent = c; }
      sel.appendChild(opt);
    });
    if (!sel.options.length) {
      const opt = document.createElement('option');
      opt.value = ''; opt.textContent = '— No active columns —'; opt.disabled = true; opt.selected = true; sel.appendChild(opt);
    }
  } catch (e) {
    ...
    if (sel.options.length) {
      const def = document.createElement('option'); def.value=''; def.textContent='--'; def.selected = true; sel.insertBefore(def, sel.firstChild);
    } else {
      const opt = document.createElement('option'); opt.value=''; opt.textContent='No active columns'; opt.disabled = true; opt.selected = true; sel.appendChild(opt);
    }
  }
  ...
  seedValTimer = setTimeout(async () => {
    try {
      ...
      (payload.items || []).forEach(it => {
        ...
        } else { const v = String(it ?? ''); o.value = v; }
        list.appendChild(o);
      });
    } catch {}
  }, 200);
...
export function initSeedsUI() {
  ...
  qs('#clear-seeds').addEventListener('click', () => { state.seeds = []; renderSeedList(); });
  qs('#seed-val').addEventListener('input', refreshSeedValuesSuggestions);
  const seedColSelect = qs('#seed-col-select');
  if (seedColSelect) seedColSelect.addEventListener('change', ()=>{ qs('#seed-val').value=''; refreshSeedValuesSuggestions(); });
