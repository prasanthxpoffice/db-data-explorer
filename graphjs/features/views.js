...
export function getSelectedViewIds() {
  return Array.from(state.viewSelected).sort((a, b) => a - b);
}

function viewLabelFor(v, lang) {
  return lang === "ar" ? v.descriptionAr || v.nameAr || `View ${v.id}` : v.descriptionEn || v.nameEn || `View ${v.id}`;
}
...
function renderMenu() {
  ...
  state.availableViews.forEach((v) => {
    ...
    cb.checked = state.viewSelected.has(v.id);
    cb.addEventListener("change", () => {
      if (cb.checked) state.viewSelected.add(v.id);
      else state.viewSelected.delete(v.id);
      updateButton();
      document.dispatchEvent(new CustomEvent("views:changed"));
    });
    const span = document.createElement("span");
    span.textContent =
      lang === "ar" ? v.descriptionAr || v.nameAr || `View ${v.id}` : v.descriptionEn || v.nameEn || `View ${v.id}`;
...
export async function initViews() {
  try {
    ...
    state.availableViews = Array.isArray(arr)
      ? arr.map((v) => ({
          id: v.id,
          nameEn: v.nameEn,
          nameAr: v.nameAr,
